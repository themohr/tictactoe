    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while(complete == 0) {
                ...
                if(player == 2)
                {
                    Console.WriteLine("Auto generate keystroke for player 2");
                    selected = Player2(spaces);
                    Thread.Sleep(500);
                }
                else
                {
            ...
            if(complete != -1) { // Display the winner
                Console.WriteLine("The winner is player: " + complete + "!");
        ...
        private static int Player2(string[] spaces) {

            Random rand = new Random();
            int i = rand.Next(1,9);

            while(spaces[i] == "X" && spaces[i] == "O") {
                // Generate a different number
                i = rand.Next(1,9);
            }

            Console.WriteLine("player 2 selection = " + i);

            return i;
        }

        private static bool IsDraw(string[] spaces) {
